...
def test_openai_compatible_adapter_enforces_local_destination_allowlist() -> None:
    settings = ProviderSettings(
        AEGIS_PROVIDER_LOCAL_BASE_URL="http://127.0.0.1:8086/v1",
...
def test_default_settings_target_llama_server_and_build_registry() -> None:
    ...
    assert settings.AEGIS_PROVIDER_LOCAL_BASE_URL == "http://localhost:8086/v1"
    assert "http://localhost:8086/v1" in settings.provider_egress_allowlist
