"""Shared helpers for Synthetic Training Scenario tests."""
...
SCENARIO = ROOT / "scenarios" / "synthetic-training"
GOLDEN_SEEDS_PATH = SCENARIO / "golden-seeds.yaml"
EVIDENCE_PATH = SCENARIO / "expected-evidence.yaml"

# Pinned tutorial seed and horizon (see golden-seeds.yaml).
TUTORIAL_SEED = 1000
STEPS = 120
...
def event_types_for_run(seed: int, steps: int = STEPS) -> set[str]:
    runtime, _ = run_scenario(seed, steps)
    return {event.type for event in runtime.events}  # type: ignore[attr-defined]
