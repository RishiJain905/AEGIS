...
async def get_incident(
    ...
    async with PostgresUnitOfWork(get_db_session_maker()) as uow:
        ...
        await require_run_access(uow, incident.run_id, actor)
        return incident
...
@router.post(
    "/runs/{run_id}/investigation/trigger-oracle",
    dependencies=[Depends(require_permission(PermissionV1.INVESTIGATION_TRIGGER))],
)
...
@router.post(
    "/runs/{run_id}/investigation/trigger-bastion",
    dependencies=[Depends(require_permission(PermissionV1.INVESTIGATION_TRIGGER))],
)
async def trigger_bastion(
    run_id: str,
...
@router.post(
    "/runs/{run_id}/investigation/trigger-warden",
    dependencies=[Depends(require_permission(PermissionV1.INVESTIGATION_TRIGGER))],
)
async def trigger_warden(
    run_id: str,
