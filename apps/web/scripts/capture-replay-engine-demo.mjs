...
const STORAGE = process.env.AEGIS_REPLAY_STORAGE_DIR ?? '/tmp/aegis-replay-storage';
const RUN = process.env.AEGIS_REPLAY_DEMO_RUN_ID ?? 'run_01ARZ3NDEKTSV4RRFFQ69G5FD0';
...
record('replay-equivalence', 'equivalence', run(`uv run aegis-replay equivalence --run-id ${RUN}`));
record(
...
try {
  ...
  const files = [
    ...
    ['25-replay-snapshot-metadata', 'replay-snapshot-create.json', 'Snapshot metadata + checksum'],
    ['25-replay-from-events', 'replay-from-events.json', 'Replay from beginning (events only)'],
    [
      '25-replay-from-snapshot-tail',
      'replay-from-snapshot.json',
      'Replay from snapshot + event tail',
    ],
    ['25-replay-equivalence', 'replay-equivalence.json', 'Equivalence validation'],
    ['25-replay-corrupt-reject', 'replay-corrupt-reject.json', 'Corrupt snapshot rejection'],
    ['25-replay-sequence-gap', 'replay-gap-detect.json', 'Sequence gap detection'],
  ];
  ...
  writeFileSync(
    isoPath,
    evidenceHtml('Live vs replay isolation', JSON.stringify(isolation, null, 2)),
  );
  await shot('25-replay-live-isolation', `file://${isoPath}`);

  // API diagnostic
  const diagRes = await fetch(`http://127.0.0.1:8000/api/v1/replay/runs/${RUN}/diagnostic`);
...
writeFileSync('/opt/cursor/artifacts/replay-capture-steps.json', JSON.stringify(steps, null, 2));
