...
Implement **Phase 13 only**. Treat project-root `architecture.md` as a binding architecture contract and this file as the acceptance contract for the current task.
...
Connect snapshots, simulation events, graph deltas, timeline, run lifecycle, and connection health into one reliable live operator view.
...
```mermaid
flowchart LR
    P04[Phase 04] --> P13[Phase 13]
    P06[Phase 06] --> P13[Phase 13]
    P10[Phase 10] --> P13[Phase 13]
    P11[Phase 11] --> P13[Phase 13]
    P12[Phase 12] --> P13[Phase 13]
```
...
- Implement thin run start/pause/resume/stop APIs and frontend flows with idempotency.
- Bootstrap from an authoritative snapshot and apply all later state through one sequence-aware reducer.
- Integrate graph, timeline, run status, asset state, and incident placeholders.
- Implement connected, reconnecting, catching-up, gap, snapshot-resync, simulator-paused, and locally-paused states.
- Stop later-event application during a sequence gap until recovery completes.
- Batch visual notifications without falsifying semantic state or cursor.
- Persist safe route/cursor context for reopening an active run.

## 8. Explicitly out of scope

- No final detection/incident engine, agents, historical replay, or production auth.
- No independent widgets consuming and reordering the same event stream.
...
- PostgreSQL is authoritative; Redis is a durable short-term delivery mechanism.
- Delivery is at least once and consumers are idempotent.
- Sequence gaps are explicit failures requiring replay/backfill/snapshot recovery.
...
8. PostgreSQL is authoritative; Redis is a durable short-term delivery mechanism.
9. Delivery is at least once and consumers are idempotent.
10. Sequence gaps are explicit failures requiring replay/backfill/snapshot recovery.

## 11. Phase execution flow

```mermaid
flowchart LR
    I[Inputs from dependencies] --> A[Implement thin run start/pause/resume/stop APIs...]
    A --> B[Bootstrap from an authoritative snapshot and ap...]
    B --> C[Integrate graph]
    C --> D[Implement connected]
...
- apps/web/features/live-run/**
- apps/web/features/timeline/**
- apps/web/lib/realtime/**
- apps/api/aegis/runs/**
- tests/e2e/live-run.spec.ts
- tests/integration/run-api/**
...
8. Create `handoffs/13-live-command-centre-integration-HANDOFF.md` after validation and provide precise evidence for every acceptance criterion.
...
- Graph, timeline, and run status share the same last-applied sequence.
- Reconnect works without full-page refresh for supported failures.
- Pause/disconnect/catch-up states are visually distinct.
- Silent Relay produces meaningful live topology and activity changes.
...
Create `handoffs/13-live-command-centre-integration-HANDOFF.md` with these sections:
