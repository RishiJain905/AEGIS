# Phase 11 — Event Persistence and Streaming
...
Implement **Phase 11 only**. Treat project-root `architecture.md` as a binding architecture contract and this file as the acceptance contract for the current task.
...
Implement the transactional outbox relay, Redis Streams delivery, idempotent consumers, retries, dead letters, and PostgreSQL backfill.
...
```mermaid
flowchart LR
    P01[Phase 01] --> P11[Phase 11]
    P02[Phase 02] --> P11[Phase 11]
    P09[Phase 09] --> P11[Phase 11]
```
...
- Implement outbox claiming, publish acknowledgement, retry, and marking logic.
- Define stream names, retention, consumer groups, pending recovery, and DLQ records.
- Implement reusable idempotent consumers keyed by event ID and consumer identity.
- Implement PostgreSQL backfill/republication when Redis history is missing.
- Create failure harnesses for relay crash, duplicate delivery, Redis restart, poison messages, and consumer restart.
- Instrument outbox age, publish latency, consumer lag, retries, pending count, and DLQ count.
- Document at-least-once semantics and consumer obligations.

## 8. Explicitly out of scope

- No WebSocket clients, frontend reducers, Kafka, or business-specific materializers beyond test references.
- No database transaction held open across Redis network calls.
...
```mermaid
flowchart LR
    I[Inputs from dependencies] --> A[Implement outbox claiming]
    A --> B[Define stream names]
    B --> C[Implement reusable idempotent consumers keyed b...]
    C --> D[Implement PostgreSQL backfill/republication whe...]
...
- services/workers/outbox/**
- packages/event-streaming/**
- apps/api/aegis/realtime/backfill.py
...
8. Create `handoffs/11-event-persistence-and-streaming-HANDOFF.md` after validation and provide precise evidence for every acceptance criterion.
...
- Committed events cannot be permanently lost by relay failure.
- Duplicate delivery does not duplicate effects.
- Redis can be rebuilt from PostgreSQL history.
- Lag, retries, and DLQ behavior are observable.
...
Create `handoffs/11-event-persistence-and-streaming-HANDOFF.md` with these sections:
