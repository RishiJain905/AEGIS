...
Implement **Phase 12 only**. Treat project-root `architecture.md` as a binding architecture contract and this file as the acceptance contract for the current task.
...
Deliver authenticated, sequence-aware live run updates with subscriptions, reconnect recovery, backpressure, and explicit resync.
...
```mermaid
flowchart LR
    P01[Phase 01] --> P12[Phase 12]
    P02[Phase 02] --> P12[Phase 12]
    P11[Phase 11] --> P12[Phase 12]
```
...
- Implement connection lifecycle, protocol negotiation, heartbeats, auth hooks, run authorization hooks, and connection limits.
- Define subscribed, event/delta, warning, error, snapshot-required, heartbeat, and resync-complete messages.
- Accept last-applied sequence and choose stream replay, PostgreSQL backfill, or fresh snapshot.
- Implement bounded per-connection queues, slow-consumer policy, clean shutdown, and reconnect behavior.
- Create a typed TypeScript transport with jittered bounded reconnection.
- Instrument connections, message lag, queue depth, gaps, resyncs, and close reasons.
- Contract-test protocol versions in Python and TypeScript.

## 8. Explicitly out of scope

- No final OIDC provider integration, feature reducers, or Redis-as-authority.
- No accepting client run IDs without server-side authorization.
...
```mermaid
flowchart LR
    I[Inputs from dependencies] --> A[Implement connection lifecycle]
    A --> B[Define subscribed]
    B --> C[Accept last-applied sequence and choose stream ...]
    C --> D[Implement bounded per-connection queues]
...
8. Create `handoffs/12-websocket-gateway-HANDOFF.md` after validation and provide precise evidence for every acceptance criterion.
...
- Reconnect repairs state without silent gaps or reordering.
- Unauthorized subscriptions are rejected server-side.
- Slow clients cannot cause unbounded memory growth.
- The protocol is versioned and cross-language tested.
...
Create `handoffs/12-websocket-gateway-HANDOFF.md` with these sections:
