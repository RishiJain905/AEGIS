...
Implement **Phase 30 only**. Treat project-root `architecture.md` as a binding architecture contract and this file as the acceptance contract for the current task.
...
Implement production OIDC sessions, roles, resource authorization, identity propagation, and security audit.
...
```mermaid
flowchart LR
    P00[Phase 00] --> P30[Phase 30]
    P02[Phase 02] --> P30[Phase 30]
    P04[Phase 04] --> P30[Phase 30]
    P24[Phase 24] --> P30[Phase 30]
```
...
- Integrate provider-neutral OIDC authorization-code flow, secure cookies, callback/logout, CSRF, and local dev auth.
- Create user, external identity, role, assignment, and optional resource-grant persistence.
- Implement viewer, analyst, operator, scenario_author, and admin permissions across HTTP, WebSockets, approvals, publication, exports, and admin.
- Propagate authenticated actor into audit events, jobs, proposals, approvals, and commands.
- Implement frontend session, forbidden, expiry, and role-aware affordances.
- Audit login, logout, denial, role change, and privileged actions.
- Fail production startup when insecure development auth is enabled.

## 8. Explicitly out of scope

- No custom password storage, SCIM, billing, or frontend-only authorization.
- No operator identity accepted from client payload.
...
```mermaid
flowchart LR
    I[Inputs from dependencies] --> A[Integrate provider-neutral OIDC authorization-c...]
    A --> B[Create user]
    B --> C[Implement viewer]
    C --> D[Propagate authenticated actor into audit events]
...
- packages/policy/authz/**
- migrations/versions/*auth*
- tests/security/auth/**
...
8. Create `handoffs/30-authentication-and-authorization-HANDOFF.md` after validation and provide precise evidence for every acceptance criterion.
...
- Unauthorized users cannot read or mutate protected resources.
- HTTP and WebSocket authorization are consistent.
...
Create `handoffs/30-authentication-and-authorization-HANDOFF.md` with these sections:
