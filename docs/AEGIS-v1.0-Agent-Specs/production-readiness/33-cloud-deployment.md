...
Implement **Phase 33 only**. Treat project-root `architecture.md` as a binding architecture contract and this file as the acceptance contract for the current task.
...
Deploy one official production-like managed-cloud target using portable containers and infrastructure as code.
...
```mermaid
flowchart LR
    P00[Phase 00] --> P33[Phase 33]
    P02[Phase 02] --> P33[Phase 33]
    P11[Phase 11] --> P33[Phase 33]
    P12[Phase 12] --> P33[Phase 33]
    P30[Phase 30] --> P33[Phase 33]
    P31[Phase 31] --> P33[Phase 33]
    P32[Phase 32] --> P33[Phase 33]
```
...
- Create production multi-stage images for web, API, simulator, workers, and agent/ML worker roles.
- Define managed container runtime, PostgreSQL, Redis, object storage, secrets, TLS, DNS, telemetry, and backups.
- Implement IaC for dev/staging/production and remote-state guidance.
- Implement CI/CD for build, scan, provenance, push, migration job, deploy, smoke test, rollback, and approvals.
- Configure private data services, network segmentation, restricted worker egress, limits, autoscaling bounds, probes, and graceful shutdown.
- Document environment contracts, costs, backup/restore, disaster assumptions, and runbooks.
- Test backup restore and rollback in staging.

## 8. Explicitly out of scope

- No multi-region active-active, internet-scale ingestion, or Kubernetes without a measured ADR.
- No provider SDK leakage into domain/application architecture.
...
```mermaid
flowchart LR
    I[Inputs from dependencies] --> A[Create production multi-stage images for web]
    A --> B[Define managed container runtime]
    B --> C[Implement IaC for dev/staging/production and re...]
    C --> D[Implement CI/CD for build]
...
8. Create `handoffs/33-cloud-deployment-HANDOFF.md` after validation and provide precise evidence for every acceptance criterion.
...
terraform fmt -check
terraform validate
python scripts/deploy_smoke_test.py --environment staging
```
...
- Staging is reproducible from IaC and runs the complete vertical slice.
- PostgreSQL/Redis are private and secrets are external.
- Migrations are controlled and rollback is rehearsed.
- Another agent/operator can reproduce deployment from docs.
...
Create `handoffs/33-cloud-deployment-HANDOFF.md` with these sections:
