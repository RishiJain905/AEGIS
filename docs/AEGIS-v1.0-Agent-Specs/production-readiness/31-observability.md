...
Implement **Phase 31 only**. Treat project-root `architecture.md` as a binding architecture contract and this file as the acceptance contract for the current task.
...
Instrument logs, metrics, traces, health, readiness, dashboards, and correlation across all critical flows.
...
```mermaid
flowchart LR
    P00[Phase 00] --> P31[Phase 31]
    P02[Phase 02] --> P31[Phase 31]
    P11[Phase 11] --> P31[Phase 31]
    P12[Phase 12] --> P31[Phase 31]
    P14[Phase 14] --> P31[Phase 31]
    P18[Phase 18] --> P31[Phase 31]
    P19[Phase 19] --> P31[Phase 31]
```
...
- Create shared OpenTelemetry setup and context propagation for Python and TypeScript/server code.
- Standardize structured fields: trace, run, incident, agent session, sequence, service, operation, and actor.
- Instrument API, DB, outbox, Redis, WebSockets, simulation, features, inference, provider calls, tools, replay, snapshots, approvals, and reports.
- Expose distinct liveness, readiness, and dependency diagnostics.
- Define metrics for latency/errors, connection/stream lag, queues, simulation throughput, ML, agents, snapshots, replay, and approval wait.
- Implement secret/PII redaction and high-cardinality metric-label rules.
- Provide local stack/export config, dashboards, and alert examples.

## 8. Explicitly out of scope

- No commercial-tool procurement requirement, secrets/raw tokens in telemetry, or authoritative use of observability data.
- No instrumentation failure allowed to break core behavior.
...
```mermaid
flowchart LR
    I[Inputs from dependencies] --> A[Create shared OpenTelemetry setup and context p...]
    A --> B[Standardize structured fields: trace]
    B --> C[Instrument API]
    C --> D[Expose distinct liveness]
...
8. Create `handoffs/31-observability-HANDOFF.md` after validation and provide precise evidence for every acceptance criterion.
...
- Representative incidents can be traced end-to-end by shared IDs.
- Logs/traces contain no secrets.
- Health/readiness are machine-readable and semantically distinct.
- Required operational metrics are emitted and tested.
...
Create `handoffs/31-observability-HANDOFF.md` with these sections:
