...
Implement **Phase 35 only**. Treat project-root `architecture.md` as a binding architecture contract and this file as the acceptance contract for the current task.
...
Freeze, document, package, demonstrate, and release the complete AEGIS v1.0 product.
...
```mermaid
flowchart LR
    P29[Phase 29] --> P35[Phase 35]
    P33[Phase 33] --> P35[Phase 35]
    P34[Phase 34] --> P35[Phase 35]
```
...
- Phase 34 — Full-System Validation (`production-readiness/34-full-system-validation.md`) and its approved handoff
...
- Finalize quick start, local/cloud setup, operator guide, scenario authoring, provider setup, troubleshooting, security scope, and contribution docs.
- Create release checklist, semantic version, changelog, migrations, known limitations, compatibility, support, license/attribution, and security reporting policy.
- Create a deterministic demo seed/script showing live graph, ML, agents, approval, replay, cinematic mode, and after-action review.
- Verify clean-machine local install and staging deployment from published instructions.
- Create immutable release manifest for source revision, image digests, migration head, scenario version, model manifests, checksums, and SBOM.
- Write evidence-backed release notes and explicit simulation-only safety boundaries.
- Tag/package only after Phase 34 approval.

## 8. Explicitly out of scope

- No new architecture, extra scenarios for vanity, or hiding known limitations.
- No unverified claims in release notes.
...
```mermaid
flowchart LR
    I[Inputs from dependencies] --> A[Finalize quick start]
    A --> B[Create release checklist]
    B --> C[Create a deterministic demo seed/script showing...]
    C --> D[Verify clean-machine local install and staging ...]
...
8. Create `handoffs/35-v1-release-preparation-HANDOFF.md` after validation and provide precise evidence for every acceptance criterion.
...
- A clean user can run the product/demo from documentation.
- Artifacts are immutable, checksummed, and traceable.
- Release checklist links Phase 34 evidence.
- The source tree is clean and safety/non-goals are explicit.
...
Create `handoffs/35-v1-release-preparation-HANDOFF.md` with these sections:
