...
Implement **Phase 09 only**. Treat project-root `architecture.md` as a binding architecture contract and this file as the acceptance contract for the current task.
...
Implement the seeded virtual world, clock, event queue, commands, checkpoints, and deterministic consequence engine that owns scenario truth.
...
```mermaid
flowchart LR
    P01[Phase 01] --> P09[Phase 09]
    P02[Phase 02] --> P09[Phase 09]
    P08[Phase 08] --> P09[Phase 09]
```
...
- Create runs from immutable scenario versions, explicit seeds, and configuration.
- Implement serializable virtual time and event queue ordered by time, priority, and deterministic tie breaker.
- Implement run-local random streams and prohibit global/random wall-clock influence.
- Implement world state for assets, relationships, generators, hidden conditions, objectives, and effects.
- Implement start, step, advance, pause, resume, stop, checkpoint, restore, and authorized command APIs.
- Emit normalized versioned domain events and deterministic history hashes.
- Persist through repositories/unit-of-work and add crash/checkpoint recovery.

## 8. Explicitly out of scope

- No Silent Relay-specific branches, Redis/WebSockets, ML, agents, or unapproved UI mutations.
- No logical event ordering based on process scheduling or wall time.
...
```mermaid
flowchart LR
    I[Inputs from dependencies] --> A[Create runs from immutable scenario versions]
    A --> B[Implement serializable virtual time and event q...]
    B --> C[Implement run-local random streams and prohibit...]
    C --> D[Implement world state for assets]
...
- services/simulation/**
- packages/simulation-domain/**
- tests/unit/simulation/**
- tests/integration/simulation/**
- tests/golden-replays/minimal/**
- docs/simulation.md
...
8. Create `handoffs/09-deterministic-simulation-core-HANDOFF.md` after validation and provide precise evidence for every acceptance criterion.
...
- Fixed inputs produce stable normalized event histories.
- Checkpoint/restore matches uninterrupted execution.
- Commands are idempotent and carry authorization context.
- The simulator owns truth independently of frontend and agents.
...
Create `handoffs/09-deterministic-simulation-core-HANDOFF.md` with these sections:
