# Phase 10 — Operation Silent Relay Content
...
Implement **Phase 10 only**. Treat project-root `architecture.md` as a binding architecture contract and this file as the acceptance contract for the current task.
...
Author the flagship multi-cause scenario that exercises the complete AEGIS product loop through one deeply polished operation.
...
```mermaid
flowchart LR
    P05[Phase 05] --> P10[Phase 10]
    P08[Phase 08] --> P10[Phase 10]
    P09[Phase 09] --> P10[Phase 10]
```
...
- Define 25–50 persistent assets across logistics, communications, identity, cloud, endpoints, databases, security, and AI systems.
- Define typed dependencies, access paths, criticality, zones, and meaningful graph clusters.
- Create normal authentication, API, database, network, deployment, health, process, and AI-use generators.
- Create seed-selected root causes: compromised credentials, approved undocumented maintenance, defective deployment, and internal misuse.
- Create overlapping early signals, clues, distractors, contradictory evidence, decision points, recurrence, and monitoring outcomes.
- Create at least three meaningful outcome branches with operational trade-offs.
- Define golden seeds, objectives, expected evidence, alternative valid responses, scoring rubric, briefing, and narrative metadata.

## 8. Explicitly out of scope

- No additional full scenarios, exploit payloads, real targets, hardcoded UI conditionals, or agent prompts.
- No unsafe real-world cyber instructions.
...
```mermaid
flowchart LR
    I[Inputs from dependencies] --> A[Define 25–50 persistent assets across logistics]
    A --> B[Define typed dependencies]
    B --> C[Create normal authentication]
    C --> D[Create seed-selected root causes: compromised c...]
...
- Silent Relay package
- golden seed registry
- expected-evidence manifest
- scoring rubric
- narrative/presentation metadata
...
- tests/scenarios/operation-silent-relay/**
- tests/golden-replays/operation-silent-relay/**
- docs/scenarios/silent-relay.md
...
8. Create `handoffs/10-operation-silent-relay-HANDOFF.md` after validation and provide precise evidence for every acceptance criterion.
...
- Every cause is solvable but not obvious from one signal.
- Golden seeds cover each cause and major branch deterministically.
- Evidence supports the truth and contradicts plausible false hypotheses.
- The package exercises graph, ML, agents, approval, replay, and scoring contracts.
...
Create `handoffs/10-operation-silent-relay-HANDOFF.md` with these sections:
