...
Implement **Phase 29 only**. Treat project-root `architecture.md` as a binding architecture contract and this file as the acceptance contract for the current task.
...
Implement deterministic explainable scoring and the final after-action learning experience.
...
```mermaid
flowchart LR
    P10[Phase 10] --> P29[Phase 29]
    P23[Phase 23] --> P29[Phase 29]
    P25[Phase 25] --> P29[Phase 29]
    P26[Phase 26] --> P29[Phase 29]
```
...
- Implement rubric-versioned scoring for detection delay, evidence coverage, hypothesis quality, false-positive cost, response proportionality, service impact, recurrence, and objectives.
- Keep numeric scores deterministic and separate from optional generated coaching.
- Attach score provenance to events, evidence, decisions, outcomes, rubric, and scenario version.
- Build after-action dashboard with overall/component scores, decisions, missed evidence, valid alternatives, affected systems, and lessons.
- Link feedback directly to replay bookmarks.
- Reveal hidden cause only after scenario completion.
- Integrate SCRIBE report/export and repeated-run comparison.

## 8. Explicitly out of scope

- No opaque model grading, global leaderboard, or reward for aggressive containment regardless of cost.
- No hidden-truth exposure during active play.
...
- Cinematic and narrative layers are derived from authoritative structured data.
- Three.js cannot invent or modify domain state and is not the primary analysis tool.
- Generated prose and visual drama may not reorder, fabricate, or hide factual chronology.
...
8. Cinematic and narrative layers are derived from authoritative structured data.
9. Three.js cannot invent or modify domain state and is not the primary analysis tool.
10. Generated prose and visual drama may not reorder, fabricate, or hide factual chronology.

## 11. Phase execution flow

```mermaid
flowchart LR
    I[Inputs from dependencies] --> A[Implement rubric-versioned scoring for detectio...]
    A --> B[Keep numeric scores deterministic and separate ...]
    B --> C[Attach score provenance to events]
    C --> D[Build after-action dashboard with overall/compo...]
...
- services/scoring/**
- apps/web/features/after-action/**
- packages/scoring-contracts/**
- tests/scoring/**
- tests/e2e/after-action.spec.ts
- docs/scoring.md
...
8. Create `handoffs/29-scoring-and-after-action-experience-HANDOFF.md` after validation and provide precise evidence for every acceptance criterion.
...
- Golden branches produce reproducible attributed scores.
- Alternative valid evidence-grounded responses can score well.
- Every component is explainable from source records.
- After-action review teaches what happened and why choices mattered.
...
Create `handoffs/29-scoring-and-after-action-experience-HANDOFF.md` with these sections:
