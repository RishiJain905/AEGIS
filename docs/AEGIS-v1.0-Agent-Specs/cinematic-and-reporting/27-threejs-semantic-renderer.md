...
Implement **Phase 27 only**. Treat project-root `architecture.md` as a binding architecture contract and this file as the acceptance contract for the current task.
...
Create a read-only React Three Fiber/Three.js adapter over the same semantic graph and replay state.
...
```mermaid
flowchart LR
    P05[Phase 05] --> P27[Phase 27]
    P06[Phase 06] --> P27[Phase 27]
    P07[Phase 07] --> P27[Phase 27]
    P25[Phase 25] --> P27[Phase 27]
```
...
- Map canonical nodes, edges, clusters, risk, activity, incidents, agents, and replay cursor into derived scene state.
- Use instanced nodes/shared materials, efficient edges/particles, cluster spatialization, and camera focus.
- Preserve deterministic/stable positions across replay and mode switches.
- Bridge selection between 2D and 3D using stable entity IDs.
- Implement quality tiers, capability detection, reduced motion, pause-when-hidden, and graceful 2D fallback.
- Dispose geometries, materials, textures, listeners, and frame loops correctly.
- Add target-size performance tests.

## 8. Explicitly out of scope

- No authoritative 3D-only facts, domain mutation, cinematic story direction, or photorealistic asset project.
- No replacing the 2D operational graph as the default analysis tool.
...
```mermaid
flowchart LR
    I[Inputs from dependencies] --> A[Map canonical nodes]
    A --> B[Use instanced nodes/shared materials]
    B --> C[Preserve deterministic/stable positions across ...]
    C --> D[Bridge selection between 2D and 3D using stable...]
...
- SemanticSceneAdapter
- derived SceneNode/SceneEdge
- CameraBookmark3D
- RenderQualityTier
- CapabilityReport
- selection bridge
...
- apps/web/features/cinematic-graph/**
- apps/web/features/cinematic-graph/adapters/**
- apps/web/features/cinematic-graph/shaders/**
- tests/unit/cinematic/**
- tests/performance/cinematic/**
- docs/frontend/cinematic-renderer.md
...
8. Create `handoffs/27-threejs-semantic-renderer-HANDOFF.md` after validation and provide precise evidence for every acceptance criterion.
...
- 2D and 3D show the same semantic state at the same cursor.
- Unsupported devices fall back cleanly.
- Mode switching preserves selection and leaks no GPU resources.
...
Create `handoffs/27-threejs-semantic-renderer-HANDOFF.md` with these sections:
