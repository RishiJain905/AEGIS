...
Implement **Phase 28 only**. Treat project-root `architecture.md` as a binding architecture contract and this file as the acceptance contract for the current task.
...
Create directed camera chapters and presentation effects tied to authoritative replay events.
...
```mermaid
flowchart LR
    P26[Phase 26] --> P28[Phase 28]
    P27[Phase 27] --> P28[Phase 28]
```
...
- Define cinematic beats/chapters referencing sequence/time, entities, paths, incidents, agents, proposals, and actions.
- Implement camera director, establishing views, path traces, agent investigation focus, approval moment, consequence reveal, and final overview.
- Generate default beats from important event types and accept safe scenario presentation hints.
- Implement play, pause, skip, chapter navigation, free-camera takeover, resume, speed, captions, and reduced-motion alternatives.
- Maintain deterministic beat ordering and handle missing references gracefully.
- Allow every beat to open the exact corresponding 2D replay cursor.
- Prevent presentation hints from revealing hidden truth before evidence.

## 8. Explicitly out of scope

- No fabricated/reordered events, prerendered video export, or cinematic-only analysis requirements.
- No drama that changes factual chronology.
...
```mermaid
flowchart LR
    I[Inputs from dependencies] --> A[Define cinematic beats/chapters referencing seq...]
    A --> B[Implement camera director]
    B --> C[Generate default beats from important event typ...]
    C --> D[Implement play]
...
- apps/web/features/cinematic-replay/**
- packages/cinematic-contracts/**
- scenarios/operation-silent-relay/presentation/**
- tests/e2e/cinematic-replay.spec.ts
- docs/frontend/cinematic-replay.md
...
8. Create `handoffs/28-cinematic-incident-replay-HANDOFF.md` after validation and provide precise evidence for every acceptance criterion.
...
- A complete Silent Relay run plays as coherent chapters.
- Every beat is linked to authoritative cursor/entity provenance.
- Users can jump to equivalent 2D analysis state.
- Captions/reduced motion preserve essential meaning.
...
Create `handoffs/28-cinematic-incident-replay-HANDOFF.md` with these sections:
