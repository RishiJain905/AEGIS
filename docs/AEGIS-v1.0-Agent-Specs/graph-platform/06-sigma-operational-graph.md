# Phase 06 — Sigma.js Operational Graph
...
Implement **Phase 06 only**. Treat project-root `architecture.md` as a binding architecture contract and this file as the acceptance contract for the current task.
...
Build the primary 2D operational investigation graph with search, filtering, selection, path tracing, evidence overlays, and progressive disclosure.
...
```mermaid
flowchart LR
    P03[Phase 03] --> P06[Phase 06]
    P04[Phase 04] --> P06[Phase 06]
    P05[Phase 05] --> P06[Phase 06]
```
...
- Integrate Sigma.js with the graph-domain store and command-centre shell.
- Implement pan, zoom, fit, focus, hover, selection, keyboard alternatives, search, filters, legends, and layer controls.
- Implement neighborhood isolation, shortest-path highlighting, dependency paths, cluster boundaries, and incident-focused mode.
- Centralize semantic styling for entity type, risk, criticality, status, confidence, activity, and inferred edges.
- Link selected graph entities to asset, evidence, incident, agent, and path inspector sections.
- Batch graph mutations and preserve stable placement for new and existing nodes.
- Implement reduced-motion behavior and accessibility alternatives for non-pointer users.

## 8. Explicitly out of scope

- No per-node React components, live streaming integration, Three.js, or risk calculation.
- No graph facts invented by visual effects.
...
- Graph domain truth is independent of Sigma.js and Three.js.
- Do not render one React component per graph entity.
- Preserve stable IDs and positions; dense views require progressive disclosure rather than indiscriminate edge rendering.
...
8. Graph domain truth is independent of Sigma.js and Three.js.
9. Do not render one React component per graph entity.
10. Preserve stable IDs and positions; dense views require progressive disclosure rather than indiscriminate edge rendering.

## 11. Phase execution flow

```mermaid
flowchart LR
    I[Inputs from dependencies] --> A[Integrate Sigma.js with the graph-domain store ...]
    A --> B[Implement pan]
    B --> C[Implement neighborhood isolation]
    C --> D[Centralize semantic styling for entity type]
...
- apps/web/features/operational-graph/**
- apps/web/features/inspector/**
- packages/ui/src/graph-controls/**
- tests/e2e/operational-graph.spec.ts
...
8. Create `handoffs/06-sigma-operational-graph-HANDOFF.md` after validation and provide precise evidence for every acceptance criterion.
...
- A user can search, isolate, trace, inspect, and restore the full graph.
- Visual state derives only from semantic contracts.
- Keyboard/reduced-motion alternatives remain useful.
- Unmounting and switching views cleans renderer resources.
...
Create `handoffs/06-sigma-operational-graph-HANDOFF.md` with these sections:
