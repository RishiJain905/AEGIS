...
Implement **Phase 05 only**. Treat project-root `architecture.md` as a binding architecture contract and this file as the acceptance contract for the current task.
...
Implement renderer-independent graph state, delta application, consistency rules, filtering, and graph algorithms with Graphology.
...
- Create canonical-to-Graphology adapters without leaking Graphology types to consumers.
- Implement snapshot load, idempotent sequence/revision-aware delta application, stale rejection, and gap detection.
- Implement node/edge lifecycle and consistency validation.
- Implement neighborhoods, k-hop expansion, directed/undirected paths, typed relationship filters, dependencies, clusters, components, and incident subgraphs.
- Implement graph-layer and predicate filtering without deleting canonical state.
- Define deterministic path-query ordering and explanation data.
- Create realistic fixtures and performance baselines for thousands of elements.

## 8. Explicitly out of scope

- No Sigma.js, Three.js, React, persistent projection workers, or graph-risk propagation.
- No renderer-owned domain state.
...
```mermaid
flowchart LR
    I[Inputs from dependencies] --> A[Create canonical-to-Graphology adapters without...]
    A --> B[Implement snapshot load]
    B --> C[Implement node/edge lifecycle and consistency v...]
    C --> D[Implement neighborhoods]
...
- tests/unit/graph-domain/**
- tests/performance/graph-domain/**
- docs/graph-domain.md
...
8. Create `handoffs/05-graph-domain-engine-HANDOFF.md` after validation and provide precise evidence for every acceptance criterion.
...
- Duplicate or out-of-order deltas cannot silently corrupt state.
- Algorithms return deterministic explainable results.
- The package has no React or renderer dependency.
- The same semantic graph can feed 2D, 3D, replay, and analysis.
...
Create `handoffs/05-graph-domain-engine-HANDOFF.md` with these sections:
