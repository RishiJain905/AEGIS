# Phase 07 — Graph Performance and Worker Layouts
...
Implement **Phase 07 only**. Treat project-root `architecture.md` as a binding architecture contract and this file as the acceptance contract for the current task.
...
Harden the 2D graph for hundreds to low thousands of elements using worker layouts, incremental placement, level of detail, and measurable budgets.
...
```mermaid
flowchart LR
    P05[Phase 05] --> P07[Phase 07]
    P06[Phase 06] --> P07[Phase 07]
```
...
- Move ForceAtlas2 and expensive calculations to versioned Web Worker protocols.
- Implement cancellation and stale-result rejection by graph revision.
- Implement incremental seeding, cluster-aware placement, pinning, persisted positions, and controlled relayout triggers.
- Implement batched updates, frame-budget scheduling, label culling, edge reduction, and cluster collapsing.
- Create deterministic target/stress graph generators using realistic heterogeneous topology.
- Instrument frame time, update latency, worker duration, memory, and dropped work.
- Document target budgets and graceful degradation tiers.

## 8. Explicitly out of scope

- No semantic-contract redesign, backend scale project, 3D optimization, or internet-scale claims.
- No global layout restart for every small live delta.
...
```mermaid
flowchart LR
    I[Inputs from dependencies] --> A[Move ForceAtlas2 and expensive calculations to ...]
    A --> B[Implement cancellation and stale-result rejecti...]
    B --> C[Implement incremental seeding]
    C --> D[Implement batched updates]
...
- apps/web/workers/layout/**
- apps/web/features/operational-graph/performance/**
- tests/performance/graph/**
- scripts/generate_graph_fixture.*
...
8. Create `handoffs/07-graph-performance-and-worker-layouts-HANDOFF.md` after validation and provide precise evidence for every acceptance criterion.
...
- Target fixtures remain interactive within documented budgets.
- Small updates preserve the user’s mental map.
- Workers and renderers do not leak resources over long sessions.
- Degradation preserves investigation usefulness.
...
Create `handoffs/07-graph-performance-and-worker-layouts-HANDOFF.md` with these sections:
