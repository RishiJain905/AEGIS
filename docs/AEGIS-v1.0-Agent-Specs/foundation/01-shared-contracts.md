...
Implement **Phase 01 only**. Treat project-root `architecture.md` as a binding architecture contract and this file as the acceptance contract for the current task.
...
Define canonical versioned contracts shared by Python, TypeScript, persistence, realtime, simulation, graph, ML, agents, and UI.
...
- Implement Pydantic and TypeScript runtime-validation packages with equivalent semantics.
- Define stable authored IDs, runtime IDs, UTC timestamps, simulation timestamps, sequence, revision, trace, causation, and correlation fields.
- Define DomainEventEnvelope v1 and an event-type registry.
- Define graph node, edge, snapshot, delta, cluster, path-query, and path-result contracts.
- Define base records for scenarios, runs, alerts, incidents, evidence, hypotheses, agent sessions, proposals, approvals, actions, model manifests, and scores.
- Define API errors, pagination, idempotency metadata, and protocol versioning.
- Create cross-language golden fixtures, JSON Schemas, round-trip tests, and compatibility checks.

## 8. Explicitly out of scope

- No ORM models, route handlers, renderer-specific fields, or business services.
- No independent duplicate contracts inside feature packages.
...
```mermaid
flowchart LR
    I[Inputs from dependencies] --> A[Implement Pydantic and TypeScript runtime-valid...]
    A --> B[Define stable authored IDs]
    B --> C[Define DomainEventEnvelope v1 and an event-type...]
    C --> D[Define graph node]
...
- DomainEventEnvelope v1
- GraphNode/GraphEdge/GraphSnapshot/GraphDelta v1
- API error/pagination v1
- base scenario/run/incident/evidence/agent/proposal/model contracts
- schema compatibility policy

Contract rules:

- This phase defines the canonical primitives. Establish one source of truth and prohibit later packages from redeclaring them.
...
- tests/contract/test_cross_language_*
- docs/contracts/versioning.md
- scripts/check_contract_compatibility.*
...
8. Create `handoffs/01-shared-contracts-HANDOFF.md` after validation and provide precise evidence for every acceptance criterion.
...
- Python and TypeScript round-trip golden fixtures without semantic loss.
- Contracts have no UI, ORM, provider, or renderer dependencies.
- Breaking changes are detected automatically.
- Unknown schema versions and invalid identifiers fail with structured errors.
...
Create `handoffs/01-shared-contracts-HANDOFF.md` with these sections:
