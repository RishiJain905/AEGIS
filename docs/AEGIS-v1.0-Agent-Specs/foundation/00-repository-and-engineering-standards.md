# Phase 00 — Repository and Engineering Standards
...
Implement **Phase 00 only**. Treat project-root `architecture.md` as a binding architecture contract and this file as the acceptance contract for the current task.
...
Create the reproducible mixed TypeScript/Python monorepo, local tooling, CI, containers, and engineering rules required by every later phase.
...
```mermaid
flowchart LR
    A[Architecture baseline] --> P00[Phase 00]
```

### Direct dependencies

None. This is a root phase.
...
- Create the architecture-defined repository tree and package ownership boundaries.
- Configure pnpm workspaces, strict TypeScript, ESLint, Prettier, and reproducible Node version pinning.
- Configure uv, Python version pinning, Ruff, mypy, pytest, and coverage.
- Add root task orchestration for bootstrap, format, lint, typecheck, test, build, integration, and clean.
- Add Dockerfiles and Docker Compose services for web, API, worker, simulator, PostgreSQL, Redis, and S3-compatible local storage.
- Add GitHub Actions for static checks, tests, builds, secret/dependency scans, and container verification.
- Add environment validation, `.env.example`, engineering standards, ADR rules, and agent handoff conventions.

## 8. Explicitly out of scope

- No domain behavior, scenario logic, graph implementation, ML, agent roles, auth, or provider-specific cloud deployment.
- No Kubernetes and no production-ready service claims from placeholder processes.
...
- Choose boring, well-supported tooling and pin versions. Foundational convenience must not create a hidden platform dependency.
- Package boundaries and dependency direction are part of the deliverable, not documentation-only suggestions.
- Generated artifacts must be reproducible and clearly separated from source.
...
8. Choose boring, well-supported tooling and pin versions. Foundational convenience must not create a hidden platform dependency.
9. Package boundaries and dependency direction are part of the deliverable, not documentation-only suggestions.
10. Generated artifacts must be reproducible and clearly separated from source.

## 11. Phase execution flow

```mermaid
flowchart LR
    I[Inputs from dependencies] --> A[Create the architecture-defined repository tree...]
    A --> B[Configure pnpm workspaces]
    B --> C[Configure uv]
    C --> D[Add root task orchestration for bootstrap]
...
- This phase defines engineering and workspace contracts only; it must not invent the domain primitives owned by Phase 01.
- Toolchain, environment, task, and package-boundary conventions must be machine-checkable where practical.
- CI and local task names must remain stable because later phase specifications and branch protection depend on them.
- Generated configuration must be reproducible and documented.
- Any change to dependency direction or repository layout after Phase 00 requires explicit migration notes and, when architectural, an ADR.
...
8. Create `handoffs/00-repository-and-engineering-standards-HANDOFF.md` after validation and provide precise evidence for every acceptance criterion.
...
- A clean checkout bootstraps using one documented workflow.
- Frozen installs and all root quality commands pass locally and in CI.
- Repository boundaries match `architecture.md` and cycles fail CI.
...
Create `handoffs/00-repository-and-engineering-standards-HANDOFF.md` with these sections:
