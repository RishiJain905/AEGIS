...
Implement **Phase 02 only**. Treat project-root `architecture.md` as a binding architecture contract and this file as the acceptance contract for the current task.
...
Build PostgreSQL persistence, migrations, repository interfaces, transaction boundaries, event history, and the transactional outbox.
...
- Configure SQLAlchemy 2 async access, Alembic, connection settings, and health checks.
- Create tables for scenarios, versions, runs, assets, relationships, events, outbox, alerts, incidents, evidence, hypotheses, agents, tools, proposals, approvals, actions, models, objects, and snapshots.
- Implement repository protocols and PostgreSQL adapters that return domain objects rather than raw ORM entities.
- Implement unit-of-work semantics so state, events, and outbox rows commit atomically.
- Add uniqueness, foreign keys, optimistic revisions, idempotency records, and replay/query indexes.
- Create deterministic local seed infrastructure and real-PostgreSQL integration test helpers.
- Document JSONB boundaries, migration policy, backups, and transaction ownership.

## 8. Explicitly out of scope

- No Redis publication, WebSockets, feature logic, scenario execution, auth, or direct route-to-table writes.
- No SQLite substitution for release-critical integration tests.
...
```mermaid
flowchart LR
    I[Inputs from dependencies] --> A[Configure SQLAlchemy 2 async access]
    A --> B[Create tables for scenarios]
    B --> C[Implement repository protocols and PostgreSQL a...]
    C --> D[Implement unit-of-work semantics so state]
...
- UnitOfWork
- event append/outbox transaction
- idempotency record
...
8. Create `handoffs/02-database-foundation-HANDOFF.md` after validation and provide precise evidence for every acceptance criterion.
...
- A failed transaction leaves no partial state or orphan outbox event.
- The entire schema is reproducible from migrations.
- Repositories hide ORM details.
- Required uniqueness/index/concurrency behavior is integration-tested.
...
Create `handoffs/02-database-foundation-HANDOFF.md` with these sections:
