# Phase 03 — Command-Centre Design System
...
Implement **Phase 03 only**. Treat project-root `architecture.md` as a binding architecture contract and this file as the acceptance contract for the current task.
...
Create AEGIS visual language, accessible primitives, data-density rules, semantic status styling, and reduced-motion behavior.
...
- Define tokens for surfaces, typography, spacing, borders, depth, focus, risk, status, and density.
- Build accessible primitives and composed panels, rails, cards, dialogs, drawers, menus, badges, alerts, skeletons, tables, metric tiles, and timeline marks.
- Define semantic icons/shapes so risk and state never depend only on color.
- Define motion duration/easing and global reduced-motion rules.
- Build Storybook or equivalent component catalogue with all important states.
- Define responsive behavior for large desktop, laptop, and constrained layouts.
- Document composition rules to prevent one-off agent-created styling systems.

## 8. Explicitly out of scope

- No operational graph, server data, simulation, incidents, agents, replay, or Three.js.
- No separate marketing-site design system.
...
```mermaid
flowchart LR
    I[Inputs from dependencies] --> A[Define tokens for surfaces]
    A --> B[Build accessible primitives and composed panels]
    B --> C[Define semantic icons/shapes so risk and state ...]
    C --> D[Define motion duration/easing and global reduce...]
...
- design tokens
- semantic risk/status presentation
...
8. Create `handoffs/03-command-centre-design-system-HANDOFF.md` after validation and provide precise evidence for every acceptance criterion.
...
- Core components pass keyboard and automated accessibility tests.
- Reduced-motion mode preserves all essential information.
- Later feature screens can be assembled without inventing foundational primitives.
- Tokens and component contracts are documented and typed.
...
Create `handoffs/03-command-centre-design-system-HANDOFF.md` with these sections:
