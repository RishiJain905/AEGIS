...
Implement **Phase 21 only**. Treat project-root `architecture.md` as a binding architecture contract and this file as the acceptance contract for the current task.
...
Implement competing evidence-grounded hypotheses, contradictions, verification requests, revisions, and confidence handling.
...
```mermaid
flowchart LR
    P19[Phase 19] --> P21[Phase 21]
    P20[Phase 20] --> P21[Phase 21]
```
...
- Define hypothesis claim, family, confidence, assumptions, supporting evidence, contradicting evidence, unknowns, predictions, and verification requests.
- Generate multiple plausible hypotheses when evidence is ambiguous.
- Validate every factual claim against visible evidence.
- Create contradiction/support matrix and comparison artifacts.
- Allow bounded focused TRACE follow-ups and update hypothesis revisions rather than overwriting history.
- Implement confidence range/coverage/contradiction checks.
- Create correct, partial, unsupported, and ambiguous Silent Relay evaluation fixtures.

## 8. Explicitly out of scope

- No action proposals, hidden chain-of-thought, hidden scenario truth, or single-shot unverified conclusions.
...
```mermaid
flowchart LR
    I[Inputs from dependencies] --> A[Define hypothesis claim]
    A --> B[Generate multiple plausible hypotheses when evi...]
    B --> C[Validate every factual claim against visible ev...]
    C --> D[Create contradiction/support matrix and compari...]
...
- services/agents/roles/oracle/**
- services/agents/tools/hypothesis/**
- tests/agents/oracle/**
- fixtures/model-responses/oracle/**
...
8. Create `handoffs/21-oracle-HANDOFF.md` after validation and provide precise evidence for every acceptance criterion.
...
- Outputs are structured, revisable, and evidence-linked.
- Unsupported facts are rejected or labeled as assumptions/inferences.
- Contradictions and unknowns remain visible.
- No hidden truth reaches the model context.
...
Create `handoffs/21-oracle-HANDOFF.md` with these sections:
