...
Implement **Phase 23 only**. Treat project-root `architecture.md` as a binding architecture contract and this file as the acceptance contract for the current task.
...
Implement structured evidence-linked incident and after-action reporting with grounded model-assisted narrative and deterministic fallback.
...
```mermaid
flowchart LR
    P19[Phase 19] --> P23[Phase 23]
    P20[Phase 20] --> P23[Phase 23]
    P21[Phase 21] --> P23[Phase 23]
    P22[Phase 22] --> P23[Phase 23]
```
...
- Define authoritative report source and report JSON for chronology, affected assets, detections, evidence, hypotheses, decisions, actions, outcomes, scores, and lessons.
- Assemble report sources deterministically from authoritative records.
- Generate optional narrative constrained to source facts and citation IDs.
- Validate all event/evidence/hypothesis/proposal/action/score citations.
- Persist immutable report versions, model/provider metadata, source sequence range, and export artifacts.
- Provide deterministic template-only fallback.
- Support Markdown, JSON, and HTML export.

## 8. Explicitly out of scope

- No report prose as authoritative truth, no unsupported recommendations, and no private chain-of-thought.
- No dependency on an external model for basic reports.
...
```mermaid
flowchart LR
    I[Inputs from dependencies] --> A[Define authoritative report source and report J...]
    A --> B[Assemble report sources deterministically from ...]
    B --> C[Generate optional narrative constrained to sour...]
    C --> D[Validate all event/evidence/hypothesis/proposal...]
...
- AfterActionReportSource
- AfterActionReport v1
- ReportCitation
- ReportVersion
...
- services/agents/roles/scribe/**
- services/reports/**
- packages/report-contracts/**
- templates/reports/**
- tests/reports/**
- docs/reports.md
...
8. Create `handoffs/23-scribe-HANDOFF.md` after validation and provide precise evidence for every acceptance criterion.
...
- Complete runs produce valid structured and readable reports.
- Every factual statement resolves to source records.
- Grounding failure falls back safely.
...
Create `handoffs/23-scribe-HANDOFF.md` with these sections:
