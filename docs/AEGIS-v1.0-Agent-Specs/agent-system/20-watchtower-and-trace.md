...
Implement **Phase 20 only**. Treat project-root `architecture.md` as a binding architecture contract and this file as the acceptance contract for the current task.
...
Implement alert triage/correlation and bounded evidence gathering that expands the investigation graph.
...
```mermaid
flowchart LR
    P13[Phase 13] --> P20[Phase 20]
    P15[Phase 15] --> P20[Phase 20]
    P17[Phase 17] --> P20[Phase 20]
    P19[Phase 19] --> P20[Phase 20]
```
...
- Implement WATCHTOWER triggers, task schemas, alert summaries, correlation recommendations, escalation, and rationale.
- Implement TRACE plans, bounded searches, evidence collection, contradiction collection, and affected-asset candidates.
- Create read tools for events, assets, relationships, paths, scores, timelines, and existing evidence.
- Create analysis-write tools for evidence attachments and investigation notes.
- Implement evidence ranking, deduplication, visibility validation, hop/time/tool/token budgets.
- Persist why alerts were grouped or separated.
- Emit graph/incident overlays through domain events.

## 8. Explicitly out of scope

- No final root-cause ownership, response proposals, external threat intelligence, internet search, or direct state mutation.
- No discarding contradictory evidence.
...
```mermaid
flowchart LR
    I[Inputs from dependencies] --> A[Implement WATCHTOWER triggers]
    A --> B[Implement TRACE plans]
    B --> C[Create read tools for events]
    C --> D[Create analysis-write tools for evidence attach...]
...
- WatchtowerTriageResult
- TraceInvestigationPlan
- EvidenceAttachment
- CandidateAffectedAsset
- InvestigationNote
- AgentGraphOverlay
...
- services/agents/roles/watchtower/**
- services/agents/roles/trace/**
- services/agents/tools/investigation/**
- tests/agents/watchtower/**
- tests/agents/trace/**
- fixtures/model-responses/watchtower-trace/**
...
8. Create `handoffs/20-watchtower-and-trace-HANDOFF.md` after validation and provide precise evidence for every acceptance criterion.
...
- Silent Relay signals become a grounded investigation.
- Every collected fact resolves to authoritative data.
- Search and graph expansion are bounded.
- Contradictory evidence remains available to ORACLE.
...
Create `handoffs/20-watchtower-and-trace-HANDOFF.md` with these sections:
