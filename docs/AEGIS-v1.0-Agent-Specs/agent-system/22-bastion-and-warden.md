...
Implement **Phase 22 only**. Treat project-root `architecture.md` as a binding architecture contract and this file as the acceptance contract for the current task.
...
Implement proportionate response proposals and independent deterministic policy evaluation.
...
```mermaid
flowchart LR
    P15[Phase 15] --> P22[Phase 22]
    P17[Phase 17] --> P22[Phase 22]
    P19[Phase 19] --> P22[Phase 22]
    P21[Phase 21] --> P22[Phase 22]
```
...
- Implement BASTION response options with evidence basis, expected benefit, operational cost, reversibility, affected assets, prerequisites, and monitoring.
- Define abstract allowlisted simulated actions such as isolate, revoke, restrict, restart/rollback, increase monitoring, and observe.
- Implement WARDEN policy by action class, role, asset criticality, reversibility, scenario restriction, incident state, and approval requirement.
- Persist proposal revisions, policy inputs, decisions, denial reasons, and approval requirements.
- Use model-assisted WARDEN prose only as non-authoritative explanation.
- Emit incident lifecycle transitions for proposed/pending/denied states.
- Ensure policy is re-runnable and prepared for final pre-execution revalidation.

## 8. Explicitly out of scope

- No human approval UI or command execution, no real remediation, no model policy override, and no arbitrary commands.
- No direct simulator mutation by BASTION or WARDEN.
...
```mermaid
flowchart LR
    I[Inputs from dependencies] --> A[Implement BASTION response options with evidenc...]
    A --> B[Define abstract allowlisted simulated actions s...]
    B --> C[Implement WARDEN policy by action class]
    C --> D[Persist proposal revisions]
...
- services/agents/roles/bastion/**
- services/agents/roles/warden/**
- packages/policy/**
- tests/agents/bastion/**
- tests/policy/**
...
8. Create `handoffs/22-bastion-and-warden-HANDOFF.md` after validation and provide precise evidence for every acceptance criterion.
...
- Every proposal has evidence, impact, reversibility, and monitoring.
- Policy decisions are deterministic and auditable.
- Only allowlisted scenario commands may be proposed.
- Class 2/3 proposals always require human approval.
...
Create `handoffs/22-bastion-and-warden-HANDOFF.md` with these sections:
