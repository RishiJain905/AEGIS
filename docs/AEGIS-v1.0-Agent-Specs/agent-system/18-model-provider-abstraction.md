...
Implement **Phase 18 only**. Treat project-root `architecture.md` as a binding architecture contract and this file as the acceptance contract for the current task.
...
Create a provider-neutral LLM interface supporting hosted, local, fake, and recorded providers with constrained outputs and usage controls.
...
```mermaid
flowchart LR
    P00[Phase 00] --> P18[Phase 18]
    P01[Phase 01] --> P18[Phase 18]
    P02[Phase 02] --> P18[Phase 18]
```
...
- Define generation messages, tool schemas, structured-output specs, response, usage, finish, latency, and error contracts.
- Implement one hosted adapter, one OpenAI-compatible/local adapter, deterministic fake provider, and recorded-response provider.
- Implement timeout, bounded retry, backoff, circuit breaker, concurrency, cancellation, and token/cost budget hooks.
- Validate structured outputs and apply a strictly bounded repair strategy.
- Persist provider/model/config version, sanitized request/response artifacts, usage, latency, and errors.
- Create shared provider conformance tests.
- Ensure core CI makes no external model calls.

## 8. Explicitly out of scope

- No agent roles, tools, prompts, fine-tuning, or vendor SDK imports in domain code.
- No unredacted secrets or provider headers in logs.
...
```mermaid
flowchart LR
    I[Inputs from dependencies] --> A[Define generation messages]
    A --> B[Implement one hosted adapter]
    B --> C[Implement timeout]
    C --> D[Validate structured outputs and apply a strictl...]
...
- tests/agents/provider-conformance/**
- fixtures/model-responses/**
- docs/agents/model-providers.md
...
8. Create `handoffs/18-model-provider-abstraction-HANDOFF.md` after validation and provide precise evidence for every acceptance criterion.
...
- Agent code can switch providers without changes.
- Provider failures are bounded and observable.
- Fake/recorded providers support complete core CI.
- Usage and artifacts are auditable without exposing secrets.
...
Create `handoffs/18-model-provider-abstraction-HANDOFF.md` with these sections:
