...
Implement **Phase 16 only**. Treat project-root `architecture.md` as a binding architecture contract and this file as the acceptance contract for the current task.
...
Train, version, evaluate, store, load, and serve the initial Isolation Forest anomaly detector.
...
```mermaid
flowchart LR
    P02[Phase 02] --> P16[Phase 16]
    P14[Phase 14] --> P16[Phase 16]
    P15[Phase 15] --> P16[Phase 16]
```
...
- Build deterministic training data selection by scenario/run split and shared preprocessing.
- Implement Isolation Forest training and documented threshold calibration.
- Create model manifests with checksum, feature schema, datasets, code revision, hyperparameters, metrics, thresholds, and limitations.
- Store/load artifacts through object metadata and verify checksums/schema compatibility.
- Implement batch and online inference with bounded latency and graceful fallback to rules.
- Emit complete ModelScore records with structured explanation and source window.
- Compare against rules on held-out root causes and seeds.

## 8. Explicitly out of scope

- No mandatory boosted model, autoencoder, GNN, production auto-retraining, or hidden-truth features.
- No generated prose as the only explanation.
...
```mermaid
flowchart LR
    I[Inputs from dependencies] --> A[Build deterministic training data selection by ...]
    A --> B[Implement Isolation Forest training and documen...]
    B --> C[Create model manifests with checksum]
    C --> D[Store/load artifacts through object metadata an...]
...
- services/ml/models/isolation_forest/**
- services/ml/inference/**
- models/manifests/**
- models/evaluation/isolation-forest/**
...
8. Create `handoffs/16-anomaly-model-HANDOFF.md` after validation and provide precise evidence for every acceptance criterion.
...
- A checked-in manifest reproduces the model within documented tolerance.
- Offline and online scores agree for identical vectors.
- Artifact/schema mismatch fails closed.
- Rules continue functioning during model outage.
...
Create `handoffs/16-anomaly-model-HANDOFF.md` with these sections:
