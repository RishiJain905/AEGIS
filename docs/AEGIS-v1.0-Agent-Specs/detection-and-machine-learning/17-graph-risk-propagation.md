...
Implement **Phase 17 only**. Treat project-root `architecture.md` as a binding architecture contract and this file as the acceptance contract for the current task.
...
Compute deterministic bounded graph risk with direct/propagated components and explanation paths.
...
```mermaid
flowchart LR
    P05[Phase 05] --> P17[Phase 17]
    P14[Phase 14] --> P17[Phase 17]
    P15[Phase 15] --> P17[Phase 17]
    P16[Phase 16] --> P17[Phase 17]
```
...
- Define normalized inputs, relationship weights, directionality, confidence, criticality, decay, time decay, caps, and convergence.
- Implement transparent propagation over graph snapshots and incident subgraphs.
- Emit per-asset total, direct, and propagated risk.
- Record top source signals, relationship paths, weights, and decay for every material contribution.
- Implement full and safe incremental recalculation with parity tests.
- Support historical calculation at a replay sequence.
- Evaluate against affected-asset truth without using hidden truth during live inference.

## 8. Explicitly out of scope

- No GNN, automatic containment, or replacement of raw detector scores.
- No renderer-specific styling inside the risk engine.
...
```mermaid
flowchart LR
    I[Inputs from dependencies] --> A[Define normalized inputs]
    A --> B[Implement transparent propagation over graph sn...]
    B --> C[Emit per-asset total]
    C --> D[Record top source signals]
...
- packages/graph-risk/**
- tests/ml/risk/**
- models/evaluation/graph-risk/**
- docs/ml/graph-risk.md
...
8. Create `handoffs/17-graph-risk-propagation-HANDOFF.md` after validation and provide precise evidence for every acceptance criterion.
...
- Cycles cannot create runaway amplification.
- Every propagated contribution has an explanation path.
- Live and historical results agree for identical state.
- Risk remains bounded, deterministic, and decomposable.
...
Create `handoffs/17-graph-risk-propagation-HANDOFF.md` with these sections:
