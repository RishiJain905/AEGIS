...
Implement **Phase 14 only**. Treat project-root `architecture.md` as a binding architecture contract and this file as the acceptance contract for the current task.
...
Create versioned deterministic features with shared offline/online computation and full provenance.
...
```mermaid
flowchart LR
    P01[Phase 01] --> P14[Phase 14]
    P09[Phase 09] --> P14[Phase 14]
    P11[Phase 11] --> P14[Phase 14]
```
...
- Define feature schemas, units, missing values, categorical handling, and version registry.
- Implement windows for authentication, API, database, network, process, deployment, health, and AI signals.
- Define entity/window keys, watermarks, deterministic closure, and late-event policy.
- Use one transformation implementation for offline dataset generation and online consumers.
- Record source run, entity, time bounds, sequence bounds, schema version, and feature provenance.
- Create reproducible content-addressed dataset exports from scenario runs.
- Add explicit tests preventing hidden scenario truth or labels from entering observable features.

## 8. Explicitly out of scope

- No thresholds, alert logic, model training, GNN, or enterprise feature store.
- No notebook-only production feature code.
...
```mermaid
flowchart LR
    I[Inputs from dependencies] --> A[Define feature schemas]
    A --> B[Implement windows for authentication]
    B --> C[Define entity/window keys]
    C --> D[Use one transformation implementation for offli...]
...
8. Create `handoffs/14-feature-pipeline-HANDOFF.md` after validation and provide precise evidence for every acceptance criterion.
...
- Offline and online features match for identical events.
- Every vector has schema version and provenance.
- Hidden truth is absent from inference inputs.
- Dataset builds are deterministic and checksummed.
...
Create `handoffs/14-feature-pipeline-HANDOFF.md` with these sections:
