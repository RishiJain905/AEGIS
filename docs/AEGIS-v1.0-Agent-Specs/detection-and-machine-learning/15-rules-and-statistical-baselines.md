# Phase 15 — Rules and Statistical Baselines
...
> **Group:** `detection-and-machine-learning`  
...
Implement **Phase 15 only**. Treat project-root `architecture.md` as a binding architecture contract and this file as the acceptance contract for the current task.
...
Implement transparent deterministic detectors and scenario-level evaluation before learned models.
...
```mermaid
flowchart LR
    P02[Phase 02] --> P15[Phase 15]
    P14[Phase 14] --> P15[Phase 15]
```
...
- Define versioned rule registry, inputs, thresholds, severity, cooldown, explanation, and candidate contracts.
- Implement detectors for unusual login, unseen device/source, service-account misuse, data volume, unusual paths, deployment-health correlation, and alert flooding/drift.
- Implement rolling statistics and calibration using training seeds only.
- Emit structured alert candidates with observed value, baseline, threshold, source window, and entity.
- Implement deterministic duplicate suppression and cooldown.
- Evaluate precision, recall, false-positive rate, detection delay, and cause coverage using scenario-level holdouts.
- Persist evaluations and explanations without creating opaque prose-only alerts.

## 8. Explicitly out of scope

- No Isolation Forest, LLM interpretation, or final incident correlation.
- No random row split as the only evaluation design.
...
- Version features, data, models, rules, thresholds, and evaluation splits.
- Preserve offline/online parity and scenario-level holdouts.
- Hidden scenario truth is evaluation-only and cannot enter live features, evidence, or agent context.
...
8. Version features, data, models, rules, thresholds, and evaluation splits.
9. Preserve offline/online parity and scenario-level holdouts.
10. Hidden scenario truth is evaluation-only and cannot enter live features, evidence, or agent context.

## 11. Phase execution flow

```mermaid
flowchart LR
    I[Inputs from dependencies] --> A[Define versioned rule registry]
    A --> B[Implement detectors for unusual login]
    B --> C[Implement rolling statistics and calibration us...]
    C --> D[Emit structured alert candidates with observed ...]
...
- services/incidents/rules/**
- services/ml/baselines/**
- models/evaluation/rules/**
- scripts/evaluate_rules.py
- tests/ml/rules/**
- docs/ml/rule-baselines.md
...
8. Create `handoffs/15-rules-and-statistical-baselines-HANDOFF.md` after validation and provide precise evidence for every acceptance criterion.
...
- Evaluation tests use scenario-level holdouts, verify offline/online parity, preserve provenance, and detect hidden-truth leakage.
...
- Baseline detectors cover all intended scenario signal families.
- Every firing is numerically/structurally explainable.
- Evaluation uses held-out runs/seeds.
- Rule failures are isolated and do not stop the pipeline.
...
Create `handoffs/15-rules-and-statistical-baselines-HANDOFF.md` with these sections:
