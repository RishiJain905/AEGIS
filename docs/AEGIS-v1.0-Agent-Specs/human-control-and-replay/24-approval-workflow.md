...
Implement **Phase 24 only**. Treat project-root `architecture.md` as a binding architecture contract and this file as the acceptance contract for the current task.
...
Implement operator approve/reject/modify, final policy revalidation, idempotent simulator command execution, and full audit history.
...
```mermaid
flowchart LR
    P02[Phase 02] --> P24[Phase 24]
    P12[Phase 12] --> P24[Phase 24]
    P13[Phase 13] --> P24[Phase 24]
    P22[Phase 22] --> P24[Phase 24]
```
...
- Build proposal API/UI showing evidence, hypothesis, impact, reversibility, assets, policy result, approvals, and alternatives.
- Implement approve, reject, modify/request-revision, cancel, expire, and supersede flows with optimistic concurrency.
- Re-run WARDEN immediately before execution and reject stale proposals.
- Map approved allowlisted proposals to internal simulator commands.
- Use idempotency keys to prevent duplicate effects under retries/reconnects.
- Audit operator identity, comments, policy rechecks, decisions, execution attempts, results, and timestamps.
- Update incident lifecycle and live UI from authoritative events.

## 8. Explicitly out of scope

- No real-system remediation, model-visible execution tools, or client-side-only authorization.
- No production OIDC setup yet, but actor hooks must exist.
...
```mermaid
flowchart LR
    I[Inputs from dependencies] --> A[Build proposal API/UI showing evidence]
    A --> B[Implement approve]
    B --> C[Re-run WARDEN immediately before execution and ...]
    C --> D[Map approved allowlisted proposals to internal ...]
...
- apps/api/aegis/approvals/**
- apps/api/aegis/commands/**
- apps/web/features/approval/**
...
8. Create `handoffs/24-approval-workflow-HANDOFF.md` after validation and provide precise evidence for every acceptance criterion.
...
- Automated tests prove Class 2/3 cannot execute without approval.
- Double submission cannot duplicate effects.
- Modified proposals create revisions and renewed policy evaluation.
- Stale state fails closed with a clear recoverable error.
...
Create `handoffs/24-approval-workflow-HANDOFF.md` with these sections:
