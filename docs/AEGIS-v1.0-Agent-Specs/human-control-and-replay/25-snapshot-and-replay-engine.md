# Phase 25 — Snapshot and Replay Engine
...
Implement **Phase 25 only**. Treat project-root `architecture.md` as a binding architecture contract and this file as the acceptance contract for the current task.
...
Implement snapshots, deterministic projectors, historical reconstruction, cursors, state differences, and equivalence tests.
...
```mermaid
flowchart LR
    P02[Phase 02] --> P25[Phase 25]
    P09[Phase 09] --> P25[Phase 25]
    P11[Phase 11] --> P25[Phase 25]
```
...
- Define snapshot schema/version, contents, trigger policy, checksum, compression, metadata, and retention.
- Implement projectors for run, graph, incidents, evidence, risk, agents, proposals, approvals, and actions.
- Load nearest prior snapshot and apply events to target sequence/simulation time.
- Implement replay cursor, ranges, filters, incident focus, and state diff.
- Implement background snapshot jobs and corrupt/missing snapshot fallback.
- Replay persisted LLM/agent artifacts without calling models.
- Compare live final state with reconstructed final state for golden runs.

## 8. Explicitly out of scope

- No frontend controls, video recordings, Three.js, or historical model regeneration.
- No treating snapshots as the authoritative history.
...
```mermaid
flowchart LR
    I[Inputs from dependencies] --> A[Define snapshot schema/version]
    A --> B[Implement projectors for run]
    B --> C[Load nearest prior snapshot and apply events to...]
    C --> D[Implement replay cursor]
...
8. Create `handoffs/25-snapshot-and-replay-engine-HANDOFF.md` after validation and provide precise evidence for every acceptance criterion.
...
- Golden final replay equals live final projection.
- Historical replay performs zero external model calls.
- Corrupt/missing snapshots fall back safely.
- Every replay result exposes provenance and applied event range.
...
Create `handoffs/25-snapshot-and-replay-engine-HANDOFF.md` with these sections:
