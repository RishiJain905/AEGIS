...
`COMPLETE` — implementation + local validation + evidence captured. Compose OTel UI blocked by Docker overlay in this environment (local API/ops evidence substitute).
...
| `pnpm typecheck`                                                     | **PASS** (turbo, 5 packages)                                                                                                                                                                                                                   |
| `pnpm test`                                                          | **PASS** — 114 web tests incl. `lib/observability/correlation.test.ts`                                                                                                                                                                         |
| `pnpm build`                                                         | **PASS** (Next.js production build)                                                                                                                                                                                                            |
| `pnpm check-contracts`                                               | **PASS**                                                                                                                                                                                                                                       |
| `uv run ruff check .`                                                | **PASS**                                                                                                                                                                                                                                       |
| `pnpm typecheck:py`                                                  | **PASS** — 375 source files (canonical gate)                                                                                                                                                                                                   |
| `uv run mypy apps services packages`                                 | **KNOWN ISSUE** — pre-existing dual-module path for `aegis_agents.runtime.factory` (documented since Phase 19; use `pnpm typecheck:py`)                                                                                                        |
| `uv run lint-imports`                                                | **PASS** — 4 contracts kept                                                                                                                                                                                                                    |
| `uv run pytest -q` (without integration)                             | **PASS** — 915 passed, 2 skipped                                                                                                                                                                                                               |
| `docker compose up -d postgres redis minio` (spec: `object-storage`) | **BLOCKED in this environment** — Docker overlay mount fails (`failed to mount ... fstype: overlay ... err: invalid argument`). Compose service `minio` is the approved stand-in for `object-storage`. Local Postgres 16 + Redis used instead. |
| `docker compose up -d otel-collector tempo prometheus grafana`       | **BLOCKED** — same overlay limitation                                                                                                                                                                                                          |
| `uv run pytest tests/integration -q`                                 | **SKIPPED here** — compose MinIO/object-storage unavailable; core readiness/authz proven against local Postgres/Redis                                                                                                                          |

Live API probes (local uvicorn, not compose): `/live` 200 `HealthResponseV1`; `/ready` 200 with postgres/redis ok (object_storage failed + not required); `/diagnostics` 401 unauth / 403 viewer / 200 admin; Redis stop → `/ready` 503 `not_ready`, restart → 200 recovery.
...
| AC                                                             | Evidence                                                                                                                                                                                                                                                                                              |
| -------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| AC1 — Representative incidents traced end-to-end by shared IDs | `TelemetryContextV1` + header propagation (`test_correlation_context_propagation`, `test_correlation_headers_round_trip`, web `correlation.test.ts`); logs carry `traceId`/`runId`/`incidentId`; Tempo Explore guidance in dashboard + `docs/observability.md`; evidence `06-correlation-headers.png` |
| AC2 — Logs/traces contain no secrets                           | `test_redaction_of_credentials_and_sensitive_fields`; `redaction.py`; collector attribute processor; instrumentation strips prompt/token/auth attributes; evidence `09-redaction-evidence.png`                                                                                                        |
| AC3 — Health/readiness machine-readable and distinct           | Fixtures `health_response_v1` vs `ready_response_v1`; live `/live` vs `/ready`; Redis failure→503 recovery; evidence `02`–`05`, `07`; `ops-snapshot.json`                                                                                                                                             |
| AC4 — Required operational metrics emitted and tested          | `AegisMetrics` registration; label policy tests; WS/streaming bridge tests; instrumentation hooks; evidence `08-admin-metrics.png`, `10-dashboard-inventory.png`, Grafana JSON under `infra/observability/`                                                                                           |

### Visual evidence files (`docs/handoffs/evidence/phase-31/`)

| File                                       | Caption                                                         |
| ------------------------------------------ | --------------------------------------------------------------- |
| `01-ops-dashboard-overview.png`            | Local ops evidence dashboard overview (compose Grafana blocked) |
| `02-health-liveness.png`                   | Public liveness (`/live` / `/health`)                           |
| `03-readiness-dependencies.png`            | Ready response with postgres/redis/object_storage               |
| `04-protected-endpoint-authz.png`          | Diagnostics 401/403 vs admin 200                                |
| `05-dependency-failure-recovery.png`       | Redis down → not_ready; restart → ready                         |
| `06-correlation-headers.png`               | W3C `traceparent` / request / correlation headers               |
| `07-admin-diagnostics.png`                 | Admin diagnostics payload                                       |
| `08-admin-metrics.png`                     | Admin metrics snapshot                                          |
| `09-redaction-evidence.png`                | Redaction of secrets from logs/attrs                            |
| `10-dashboard-inventory.png`               | Dashboard / alert inventory note                                |
| `11-responsive-ops-view.png`               | Narrow viewport ops view                                        |
| `12-command-centre-sign-in.png`            | Live web sign-in surface                                        |
| `ops-dashboard.html` / `ops-snapshot.json` | Machine-readable capture used for screenshots                   |

Capture commands: `uv run python scripts/phase31_capture_evidence.py` then `node scripts/phase31_screenshots.mjs`.

## Confirmation: no prohibited shortcut used

Production-path observability is implemented in `packages/observability` and wired through API routes, middleware, workers, and domain hooks — not scaffolding-only stubs. Exporters fail open and do not break core behavior. Secrets are redacted; high-cardinality IDs are forbidden on metric labels. Diagnostics remain `admin:manage`-protected (Phase 30). Telemetry is not treated as authoritative state. Existing domain metrics (`StreamingMetrics` / `GatewayMetrics`) were bridged, not deleted or weakened. Tests were added rather than loosened. Phase 32–35 scope was not absorbed. Compose OTel UI evidence is blocked by Docker overlay here; local API/ops dashboard screenshots and `ops-snapshot.json` substitute. Full validation matrix above reflects executed results.
