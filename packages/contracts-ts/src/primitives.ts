...
const RUNTIME_ID_PATTERN =
  /^(evt|run|trc|inc|alt|evd|ags|prp|apr|act|mdl|scr|hyp|gen|atk|tiv|aaf|wtr|tip|eat|ino|caa|ovl|rps)_[0-9A-HJKMNP-TV-Z]{26}$/;
...
export const replaySnapshotIdSchema = z.string().superRefine((value, ctx) => {
  try {
    validateRuntimeId('rps', value);
  } catch (error) {
    if (error instanceof ContractValidationError) {
      ctx.addIssue({ code: z.ZodIssueCode.custom, message: error.message });
    }
